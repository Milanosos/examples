...
public class ImportData {
	...
	public ImportData() {
		...
		runningLayer = (EditableLayer) spatialDb.getOrCreateLayer("running",
				SimplePointEncoder.class, EditableLayerImpl.class, "lon:lat");
	}

	// Import the data from a GPX file. Boolean indicates whether data has been
	// imported before
	public void addData(File file, boolean firsttime) throws IOException,
		...
		// The garmin GPX running data contains only one track containing one
		// segment
		Trkseg track = gpx.getTrk(0).getTrkseg(0);

		// Start a new transaction
		Transaction tx = graphDb.beginTx();
		// Contains the record that was added previously (in order to create a
		// relation between the new and the previous node)
		...
		for (int i = 0; i < track.getTrkptCount(); i++) {
			...
			Coordinate to = new Coordinate(track.getTrkpt(i).getLon()
					.doubleValue(), track.getTrkpt(i).getLat().doubleValue());

			// Check whether we can find a node from which is located within a
			// distance of 20 meters
			List<GeoPipeFlow> closests = new ArrayList<GeoPipeFlow>();
			if (!firsttime) {
				closests = GeoPipeline
						.startNearestNeighborLatLonSearch(runningLayer, to,
								0.02).sort("OrthodromicDistance")
						.getMin("OrthodromicDistance").toList();
			}

			SpatialDatabaseRecord torecord = null;

			// If first time, we add all nodes. Otherwise, we check whether we
			// find a node that is close enough to the current location
			if (!firsttime && (closests.size() == 1)) {
				// Retrieve the node
				System.out.println("Using existing: "
						+ closests.get(0).getProperty("OrthodromicDistance"));
				torecord = closests.get(0).getRecord();
				// Recalculate average speed
				double previousspeed = (Double) torecord.getProperty("speed");
				int previousoccurences = (Integer) torecord
						.getProperty("occurences");
				double currentspeed = analyzer.getHorizontalSpeed(track
						.getTrkpt(i).getTime());
				...
				// New node, add it
				torecord = runningLayer.add(runningLayer.getGeometryFactory()
						.createPoint(to));
				// Set the data accordingly
				torecord.setProperty("speed", analyzer.getHorizontalSpeed(track
						.getTrkpt(i).getTime()));
				torecord.setProperty("occurences", 1);
			}

			// If a previous node is available (and they are not identical), add
			// a directed relationship between both
			if (fromrecord != null && (!fromrecord.equals(torecord))) {
				Relationship next = fromrecord.getGeomNode()
						.createRelationshipTo(torecord.getGeomNode(),
								RelTypes.NEXT);
			}
